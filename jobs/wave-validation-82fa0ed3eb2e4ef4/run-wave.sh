...
wave=$1
configuration=$2
config_dir=jobs/wave-validation-82fa0ed3eb2e4ef4
...
case $wave in
  ...
  medium)
    wave_number=3
    task_source=(--dataset terminal-bench@2.0)
    tasks=(nginx-request-logging git-multibranch pytorch-model-recovery sqlite-db-truncate)
    ;;
  *)
...
uv run --isolated --locked --python 3.12 --extra harbor \
  python "$config_dir/harbor_runner.py" run \
  --config "$config_file" \
