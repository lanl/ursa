...
import coolname
import ssl, truststore, httpx, litellm
...
if __name__ == "__main__":
    ...
    DEFAULT_MODELS = tuple(models_cfg.get("choices") or (
        "openai/gpt-5",
        "openai/o3",
        "openai/o3-mini",
    ))
    ...
    try:
        ...
        while True:
            ...
            valid_nums = ", ".join(str(i) for i in range(1, len(DEFAULT_MODELS) + 1))
            if DEFAULT_MODEL:
                print(f"Please enter {valid_nums}, a custom model, or press Enter for default.")
            else:
