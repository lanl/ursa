...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/2/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
NumSandwiches = model.addVars(
    NumSandwichTypes, vtype=GRB.CONTINUOUS, name="NumSandwiches"
)
...
model.addConstr(
    quicksum(Required[0][i] * NumSandwiches[i] for i in range(NumSandwichTypes))
    <= TotalAvailable[0],
    "Constr_1",
)
...
model.addConstr(
    quicksum(Required[1][i] * NumSandwiches[i] for i in range(NumSandwichTypes))
    <= TotalAvailable[1],
    "Constr_2",
)
# @Constraint Constr_3 @Def: The number of regular and special sandwiches produced is non-negative.
model.addConstrs(
    (NumSandwiches[i] >= 0 for i in range(NumSandwichTypes)),
    "NumSandwichesNonNegative",
)
...
model.setObjective(
    quicksum(
        NumSandwiches[i] * ProfitPerSandwich[i] for i in range(NumSandwichTypes)
    ),
    GRB.MAXIMIZE,
)
...
variables = {}
objective = []
variables["NumSandwiches"] = {
    i: NumSandwiches[i].X for i in range(NumSandwichTypes)
}
