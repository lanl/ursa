...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/6/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
NumberAdsSoorchle = model.addVar(
    vtype=GRB.INTEGER, lb=0, ub=MaxAdsSoorchle, name="NumberAdsSoorchle"
)
...
model.addConstr(
    CostZTube * NumberAdsZTube
    + CostSoorchle * NumberAdsSoorchle
    + CostWassa * NumberAdsWassa
...
model.addConstr(
    (1 - MaxFractionWassaAds) * xW <= MaxFractionWassaAds * (xZ + xS)
)
...
model.setObjective(
    ViewersZTube * xZ + ViewersSoorchle * xS + ViewersWassa * xW, GRB.MAXIMIZE
)
