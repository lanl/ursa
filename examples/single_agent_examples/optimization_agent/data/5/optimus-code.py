...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/5/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
NumberOfSeniorCitizens = model.addVar(
    vtype=GRB.INTEGER, name="NumberOfSeniorCitizens"
)
# @Variable NumberOfYoungAdults @Def: The number of young adults @Shape: ['Integer']
NumberOfYoungAdults = model.addVar(
    vtype=GRB.INTEGER, name="NumberOfYoungAdults"
)
...
model.addConstr(
    SeniorWage * NumberOfSeniorCitizens + YoungAdultWage * NumberOfYoungAdults
    <= MaxWeeklyWageBill
)
