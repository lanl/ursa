...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/10/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
PackageCount = model.addVars(
    NumPackages, vtype=GRB.INTEGER, name="PackageCount"
)
...
model.addConstrs(
    (
        quicksum(Required[i][p] * PackageCount[p] for p in range(NumPackages))
        <= Available[i]
...
model.setObjective(
    quicksum(PackageProfit[j] * PackageCount[j] for j in range(NumPackages)),
    GRB.MAXIMIZE,
)
