...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/3/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
NumberRadioAds = model.addVar(
    vtype=GRB.INTEGER, lb=MinRadioAds, ub=MaxRadioAds, name="NumberRadioAds"
)
# @Variable NumberSocialMediaAds @Def: The number of social media advertisements to be contracted @Shape: []
NumberSocialMediaAds = model.addVar(
    vtype=GRB.INTEGER, lb=MinSocialMediaAds, name="NumberSocialMediaAds"
)

# Constraints
# @Constraint Constr_1 @Def: The total cost of radio ads and social media ads cannot exceed the AdvertisingBudget.
model.addConstr(
    CostRadioAd * NumberRadioAds + CostSocialMediaAd * NumberSocialMediaAds
    <= AdvertisingBudget
)
...
model.setObjective(
    ExposureRadioAd * NumberRadioAds
    + ExposureSocialMediaAd * NumberSocialMediaAds,
    GRB.MAXIMIZE,
)
