...
# Load data
with open(
    "/Users/gaowenzhi/Desktop/optimus-OR-paper/data/new_dataset/sample_datasets/7/parameters.json",
    "r",
) as f:
    data = json.load(f)
...
NumberOfDressers = model.addVar(
    vtype=GRB.INTEGER, lb=0, name="NumberOfDressers"
)
...
model.addConstr(
    StainPerChair * NumberOfChairs + StainPerDresser * NumberOfDressers
    <= AvailableStain
)
...
model.addConstr(
    NumberOfChairs * OakPerChair + NumberOfDressers * OakPerDresser
    <= AvailableOak
)
...
model.setObjective(
    ProfitPerChair * NumberOfChairs + ProfitPerDresser * NumberOfDressers,
    GRB.MAXIMIZE,
)
