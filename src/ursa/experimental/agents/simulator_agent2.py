...
1. Ensure that a custom guide for setting up and running simulations in your workspace. 
   - For consistency with downstream users, the file should be called simulator_user_guide.md
...
1. Use available documentation and tools to understand the necessary steps to properly set up 
    and carry out a simulation run or simulation campaign. 
   - Your first resource should be a file called simulator_user_guide.md which is a custom guide created
...
2. Carry out the simulation or set up and execute a successful simulation campaign based on the user request.
   - Steps to consider:
        ...
        B) Run the simulation(s) as planned and monitor for failure modes
        ...
        C) Analyze the outputs of the simulation
            - Use simulator_user_guide.md (and any other docs) to interpret outputs.
        ...
        D) Update Documentation
            - If you learned anything new (correct flags, pitfalls, missing steps, interpretation details),
                update simulator_user_guide.md to communicate all pertinant information to future users
...
class SimulatorAgent(AgentWithTools, BaseAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.documenter.safe_codes = [
            "All standard programming languages and tools"
        ]
        ...
        if self.embedding:
            ...
            print("RAG TEST:")
            print(self.rag_agent.invoke("What is PENNANT?"))
    ...
    def _runner(self, state: State) -> State:
        response = self.runner.invoke(state)
