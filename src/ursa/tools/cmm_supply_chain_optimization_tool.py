...
def _build_output(
    *,
    ...
    """Build the canonical output dict from a solved allocation.

    ``status_label`` is the feasible-case label (e.g. ``"optimal"`` or
    ``"optimal_greedy"``).  Infeasibility statuses are derived from the
    actual solution values and override ``status_label`` when appropriate.
    """
...
def _try_ortools(
    *,
    ...
    composition_tolerance: float,
    composition_profiles: dict[str, dict[str, float]],
) -> tuple[dict[tuple[str, str], float], dict[str, float], str, dict[str, float]] | None:
    """Attempt to solve via OR-Tools MIP.  Returns ``None`` on any failure."""
    try:
        from ursa.tools.cmm_ortools_solver import solve_with_ortools
    except Exception:
        return None
    ...
    result = solve_with_ortools(
        ...
        supplier_capacities={s.name: s.capacity for s in suppliers},
        supplier_unit_costs={s.name: s.unit_cost for s in suppliers},
        supplier_risk_scores={s.name: s.risk_score for s in suppliers},
        demand=demand,
        shipping_cost=shipping_cost,
        risk_weight=risk_weight,
        unmet_penalty=unmet_penalty,
    ...
    return result.allocation, result.unmet, result.solver_status, result.supplier_max
...
def solve_cmm_supply_chain_optimization(
    ...
    # --- Try OR-Tools MIP first ---------------------------------------------
    mip_result = _try_ortools(
        markets=markets,
        suppliers=suppliers,
        demand=demand,
        shipping_cost=shipping_cost,
    ...
    if mip_result is not None:
        allocation, unmet, solver_status, supplier_max = mip_result
        logger.info("OR-Tools solver succeeded with status=%s", solver_status)
