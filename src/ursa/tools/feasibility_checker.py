...
def heuristic_feasibility_check(
    ...
    try:
        for expr in constraints:
            parsed = parse_expr(expr,local_dict=locals_map,transformations=standard_transformations,evaluate=False)
    ...
    funcs = [sp.lambdify(symbols, c, modules=["math", "numpy"]) for c in parsed_constraints]
    ...
    for _ in range(samples):
        ...
        for i, sym in enumerate(symbols):
            ...
            if typ == "integer":
                ...
                raise ValueError(f"Unknown type {variable_type[i]} for variable {variable_name[i]}")
            point[sym] = value

        # Evaluate all constraints at this point
        try:
            vals = [point[s] for s in symbols]
            cons_satisfaction = [bool(np.asarray(f(*vals)).all()) for f in funcs]
            if all(cons_satisfaction):
                # Found a feasible point
                readable = {str(k): round(v, 3) for k, v in point.items()}
                feasible_point = [v for k, v in point.items()]
    ...
    for rank, idx in enumerate(order, start=1):
        expr_text = constraints[idx]  # use the original string; easier to read than str(sympy_expr)
    ...
    return (f"No feasible solution found after {samples:,} samples. Most violated constraints (low→high satisfaction):\n " + "\n  ".join(lines))
